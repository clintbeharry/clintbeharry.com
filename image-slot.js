...
(() => {
  ...
  // present, so the sidecar fetch, the drop targets, and the empty state's
  // click-to-browse are dead authoring chrome — this flag turns them off.
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '  <button data-act="clear" title="Remove image">Clear</button></div>' +
      ...
      if (!STATIC) this._empty.addEventListener('click', () => this._input.click());
    ...
    async _ingest(file) {
      ...
      try {
        // Always encode at the full cap: slots that are dropped onto while
        // rendered small (the reel's 30%-wide drop tiles) would otherwise bake
        // a low-res image that then displays at full frame size.
        const w = MAX_DIM;
